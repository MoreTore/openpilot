...
class SteeringAccuracyTool:
  ...
  def update(self, sm):
    ...
    steer_limited_by_controls = abs(sm['carControl'].actuators.torque - sm['carOutput'].actuatorsOutput.torque) > 1e-2
      ...
      if self.cnt >= 500:
        ...
        for group, group_props in self.all_groups.items():
          if v_ego > group_props[0]:
            ...
            if len(model_points):
              self.speed_group_stats[group][accel_abs]["dpp"] += abs(model_points[0])
            if steer_limited_by_controls:
              self.speed_group_stats[group][accel_abs]["limited"] += 1
